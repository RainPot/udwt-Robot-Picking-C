...
def convert(xml_list, xml_dir, json_file, img_dir):
    ...
    for line in list_fp:
        ...
        # path = get(root, 'path')
        # 取出图片名字
        filename = root.find('frame').text + '.jpg'
        img = Image.open(img_dir + line[:-4] + '.jpg')
        width = img.size[0]
        height = img.size[1]

        ## The filename must be a number
        image_id = img_id  # 图片ID
...
if __name__ == '__main__':
    # root_path = os.getcwd()
    root_path = 'F:/dataset/水下目标抓取/train_part1/val/box/'
    img_dir = 'F:/dataset/水下目标抓取/train_part1/val/image/'
    ...
    # train data
    xml_list = xml_labels
    json_file = 'F:/dataset/水下目标抓取/train_part1/val/box/instances_val2017.json'
    convert(xml_list, xml_dir, json_file, img_dir)
    # for xml_file in xml_list:
    #     img_name = xml_file[:-4] + '.jpg'
    #     shutil.copy(os.path.join(root_path, 'JPEGImages', img_name),
